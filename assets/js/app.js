		...
		updateDashboard: function()
		{
			...
			$('.actions button').eq(1).off('click').click(function() {
				...
				if($('#exportModal .radio-btns .active').length === 0) {
					$('#exportModal .radio-btns .btn').eq(0).addClass('active');
					$(this).closest('form').find('.dropdown').toggle($('#exportModal .radio-btns .btn:eq(0)').hasClass('active'));
				}
				$('#exportModal .radio-btns .btn').off('click').click(function() {
					$('#exportModal .radio-btns .active').removeClass('active');
				...
				$('#export-submit').off('click').click(function() {
					...
					if($('#exportModal .radio-btns .btn').eq(0).hasClass('active')) {
						url += encodeURIComponent('http://'+location.host+'/lesson-planning-tool/plan.php?id=' + utils.lessonId + '&section=' + $('#exportModal .dropdown').data('selected') ) + '&filename=LessonPlan.pdf';
					}
					else {
						url += encodeURIComponent('http://'+location.host+'/lesson-planning-tool/ipg.php?id=' + utils.lessonId) + '&filename=LessonPlan.pdf';
					}
					window.open(url);
	
					_gaq.push(['_trackEvent', 'UI', 'Export']);
				});
        ...
        pushView: function(viewPath)
        {
			...
			(function(view)
            {
                TweenLite.to($('.col-middle .data'), 0.25, {autoAlpha:0, onComplete:function()
                {
                    ...
                    $('.btn-strive').empty().eq(0).html('NEED HELP? <span></span>');
