    ...
    function ($, utils, tinymce, stateManager, sidebarRight, mathBase)
{
	var coherenceFW =
	{
		...
		createMCE: function(id, onChange)
		{
			tinymce.init({
				...
				setup: function(editor)
				{
					...
					editor.ui.registry.addButton('overline', {
						tooltip: 'Overline',
						text: '¯¯',
						onAction: function() {
					...
					editor.ui.registry.addButton('frac', {
							text: 'x/y',
							onAction:function(){
								editor.insertContent('<table style="text-align:center;display:inline-table;"><tr><td style="border-bottom:1px solid"></td></tr><tr><td></td></tr></table>')
							}
					});
				}
			});
		...
		loadStandards: function(immediateTags)
		{
			...
			$('.view .main-h1 + .standards-selected > .standards-list').empty().append(
				['div', {class:'k8'}].concat(
			...
			.toDomNodes());
			$('.view .main-h1 + .standards-selected > .standards-list').append(
				['div', {class:'hs'}].concat(
            ...
            var std = _.findWhere(window.standardsData.math,{id: id}) || {};
						return ['a', {class:'standards-btn '+ (/CLUSTER/.test(id)?'cluster-btn':''), title:(std.description||'').replace('\n','')}, ['b', id]
							].toDomNodes();
				};
            ...
            var std = _.findWhere(window.standardsData.math,{id: e}) || {};
						var tagMarkup = ['a', {class:'standards-btn', title:std.description||''}, ['b', e]
							].toDomNodes();
            ...
            var std = _.findWhere(window.standardsData.math,{id: e}) || {};
              ...
              var std = _.findWhere(window.standardsData.math,{id: e}) || {};
							var tagMarkup = ['a', {class:'standards-btn cluster-btn', title:std.description||''}, ['b', e]
								].toDomNodes();
